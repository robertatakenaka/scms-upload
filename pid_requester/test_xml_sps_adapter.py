...
from pid_requester.xml_sps_adapter import PidRequesterXMLAdapter, _str_with_64_char
...
def _get_xml_adapter(xml=None):
    ...
    return PidRequesterXMLAdapter(xml_with_pre)


class PidRequesterXMLAdapterAbsentDataTest(TestCase):
    def setUp(self):
...
class PidRequesterXMLAdapterIssnsTest(TestCase):
...
class PidRequesterXMLAdapterLinksTest(TestCase):
    def setUp(self):
        xml = """
...
class PidRequesterXMLAdapterCollabTest(TestCase):
    def setUp(self):
        xml = """
            <article xmlns:xlink="http://www.w3.org/1999/xlink">
                <front>
...
class PidRequesterXMLAdapterContribGroupTest(TestCase):
    def setUp(self):
        xml = """
            <article xmlns:xlink="http://www.w3.org/1999/xlink">
                <front>
...
class PidRequesterXMLAdapterArticleTitlesTest(TestCase):
...
class PidRequesterXMLAdapterTest(TestCase):
    def _get_xml_adapter(self, eissn=None, pissn=None):
        if eissn:
            eissn = f'<issn pub-type="epub">{eissn}</issn>'
        if pissn:
...
class BasePidRequesterXMLAdapterQueryParamsTest(TestCase):
    def setUp(self):
        self.xml_adapter = _get_xml_adapter()


class PidRequesterXMLAdapterQueryParamsTest(BasePidRequesterXMLAdapterQueryParamsTest):
...
class PidRequesterXMLAdapterQueryListTest(TestCase):
    def setUp(self):
        self.xml_adapter = _get_xml_adapter()
