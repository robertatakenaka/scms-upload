...
class PkgZipBuilder:
    ...
    def build_sps_package(
        ...
        with ZipFile(sps_pkg_zip_path, "w", compression=ZIP_DEFLATED) as zf:
            ...
            self._build_sps_package_add_assets(zf)

            # add renditions (pdf) to zip
            self.text = {
                "xml_langs": self.xml_with_pre.langs,
                "pdf_langs": self._build_sps_package_add_renditions(zf),
            }
    ...
    def _build_sps_package_add_renditions(self, zf):
        """
        Busca os registros dos arquivos PDF migrados relacionados com o pacote
        e adiciona o arquivo ao novo zip, renomeando-o para o padrão SPS

        Retorna os idiomas dos pdfs
        """
        pdf_langs = []

        logging.info(self.renditions)
    ...
    def _build_sps_package_add_assets(self, zf):
        """
        A partir os xlink:href encontrados no XML, busca os registros
        dos arquivos de imagem migrados relacionados com o pacote
        e adiciona o arquivo ao novo zip, renomeando-o para o padrão SPS.
        Também atualiza o valor de xlink:href para o padrão SPS.
        """
        issue_proc = self.issue_proc
        ...
        for xml_graphic in xml_assets.items:
            try:
                ...
                # procura o arquivo que corresponde a 'xml_graphic.xlink_href'
                # no contexto do "issue"
                found = False
                for asset in issue_proc.find_asset(xml_graphic.xlink_href):
                    found = True
