...
def task_publish_article(
    ...
    try:
        ...
        for item in proc_controller.create_journal_procs(user, article.journal):
            pass

        for item in proc_controller.create_issue_procs(user, article.issue):
            pass

        for journal_proc in JournalProc.objects.filter(journal=article.journal).iterator():
            try:
                website = WebSiteConfiguration.get(
            ...
            journal_proc.publish(
                user, publish_journal, website_kind=website_kind, api_data=api_data, force_update=True,
            )

            api_data["post_data_url"] = website.api_url_issue
            IssueProc.objects.get(journal_proc=journal_proc, issue=article.issue).publish(
                user, publish_issue, website_kind=website_kind, api_data=api_data, force_update=True,
            )
