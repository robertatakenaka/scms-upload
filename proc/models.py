...
class BaseProc(CommonControlField):
    ...
    def publish(self, user, callable_publish, website_kind=None, api_data=None, force_update=None):
        ...
        operation = self.start(
            user,
            f"publish {self.migrated_data.content_type} {self} on {website_kind}"
        )
    ...
    def items_to_publish_on_public(
        cls, content_type, force_update=None, params=None
    ):
        params = params or {}
        params["migrated_data__content_type"] = content_type
    ...
    def journals_with_modified_articles(cls, collection=None, journal=None):
        for item in JournalAcronIdFile.journals_with_modified_articles(
            ...
            yield from JournalProc.objects.filter(
                collection=item.collection, journal=item.journal,
            )
...
class IssueProc(BaseProc, ClusterableModel):
    ...
    def migrate_document_records(self, user, force_update=None):
        ...
        id_file_records = list(JournalAcronIdFile.modified_articles(
        ...
        for record in id_file_records:
            try:
                logging.info(
                    f"migrate_document_records: {record.item_pid}"
                )
        ...
        operation.finish(user, completed=total_docs == done, detail={
            "total_documents": got,
