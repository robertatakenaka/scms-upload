...
from upload.controller import receive_package, receive_zip_file
from upload.models import Package, choices
from upload.tasks import task_validate_original_zip_file, task_receive_packages
...
class PackageCreateView(CreateView):
    def form_valid(self, form):
        ...
        response = receive_package(self.request.user, package)
...
class PackageZipCreateView(CreateView):
    def form_valid(self, form):
        ...
        pkgzip = form.save_all(self.request.user)

        try:
            for item in receive_zip_file(self.request.user, pkgzip):

                try:
                    package = item["package"]
                    messages.success(
                        self.request,
                        f"{item.get('xml_name')}: read",
                    )
                except KeyError:
                    messages.error(
                        self.request,
                        f"{item.get('xml_name')}: {item['error']}",
                    )
                    continue
                ...
                try:
                    ...
                    messages.success(
                        self.request,
                        f"{item.get('xml_name')}: received",
                    )
                except Exception as exc:
                    messages.error(
                        self.request,
                        f"{item.get('xml_name')}: {exc}",
                    )
            return redirect(f"/admin/upload/package?q={pkgzip.name}")
        except Exception as exc:
            messages.error(
                self.request,
                f"{pkgzip}: {exc}",
