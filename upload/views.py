...
class PackageZipCreateView(CreateView):
    def form_valid(self, form):
        ...
        try:
            messages.success(
                self.request,
                _("Received {}").format(", ".join(pkg_zip.xmls)),
            )
            messages.info(
                self.request,
                _("Validations in progress. Access Upload > Uploaded packages and search by {}").format(pkg_zip.name),
            )
...
class ReadyToPreviewPackageEditView(EditView):
    def form_valid(self, form):
        package = form.save_all(self.request.user)
