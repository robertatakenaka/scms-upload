...
def task_process_qa_decision(
    ...
    user = User.objects.get(pk=user_id)
    package = Package.objects.get(pk=package_id)
    publication = package.process_qa_decision(user)

    if package.qa_decision == choices.PS_READY_TO_PREVIEW:
        task_publish_article.apply_async(
            kwargs=dict(
                user_id=user.id,
                username=user.username,
                api_data=None,
                website_kind=publication["website_kind"],
                article_proc_id=None,
                upload_package_id=package.id,
            )
        )
    elif package.qa_decision == choices.PS_READY_TO_PUBLISH:
        if not package.pkg_zip.packages.all().filter(
            ~Q(qa_decision=publication["qa_decision"])
        ).exists():
