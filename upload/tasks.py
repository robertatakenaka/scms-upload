...
def task_receive_packages(
    self, user_id, pkgzip_id
):
    user = User.objects.get(pk=user_id)
    # pkgzip = PackageZip.objects.get(pk=pkgzip_id)

    logging.info(pkgzip_id)

    logging.info(Package.objects.filter(pkgzip__pk=pkgzip_id).count())
    logging.info(Package.objects.filter(pkgzip__id=pkgzip_id).count())

    for package in Package.objects.filter(pkgzip__pk=pkgzip_id):
