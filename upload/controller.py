...
from upload.models import ValidationReport, XMLErrorReport, XMLInfoReport, XMLError
...
def _check_package_is_expected(response):
    ...
    try:
        ...
        try:
            previous_package = Package.objects.filter(article=article).order_by('-created').first()
            if previous_package.status in (choices.PS_REJECTED, choices.PS_PENDING_CORRECTION):
        ...
        if article.status == article_choices.AS_REQUIRE_UPDATE:
            ...
            # article.status = article_choices.AS_UPDATE_IN_PROGRESS
            # article.save()
        elif article.status == article_choices.AS_REQUIRE_ERRATUM:
            response["package_category"] = choices.PC_ERRATUM
            # article.status = article_choices.AS_CORRECTION_IN_PROGRESS
            # article.save()
        else:
            response["package_category"] = choices.PC_UPDATE
            raise PackageError(
                f"Package is rejected because the article is not marked to be updated / corrected. Article status: {article.status}"
            )
