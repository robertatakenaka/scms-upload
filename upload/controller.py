...
from packtools.sps.pid_provider.xml_sps_lib import GetXMLItemsError, XMLWithPre, get_zips
...
def receive_zip_file(user, pkg_zip):
    try:
        zip_xml_file_path = pkg_zip.file.path
        response = {}

        for item in get_zips(zip_xml_file_path):
            try:
                name = item["zipfilename"]
                content = item["content"]

                package = Package()
                package.creator = user
                package.pkgzip = pkg_zip
                package.save()
                package.save_file(name, content)

                yield {"xml_name": name, "package": package}
            except Exception as exc:
                return {"error": str(exc), "xml_name": name}
    except Exception as exc:
        return {"error": f"Unable to read {pkg_zip}"}
