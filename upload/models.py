...
class Package(CommonControlField, ClusterableModel):
    ...
    def calculate_validation_numbers(self):
        ...
        self.xml_errors_percentage = round(total_xml_errors * 100 / total_xml_issues, 2)
        self.xml_warnings_percentage = round(
            total_xml_warnings * 100 / total_xml_issues, 2
        )
        ...
        self.numbers = {
            ...
            "total_issues": total_pkg_issues + total_xml_issues,
        }
        self.save()
    ...
    def prepare_xml_file(self, xml_with_pre):
        changed = False
        try:
            self.order = int(self.order or xml_with_pre.order)
            save = True
        ...
        if not self.pid_v2:
            self.pid_v2 = xml_with_pre.v2 or self.recover_pid_v2()
            save = True
        ...
        if not self.website_pub_date or not self.approved_date:
            ...
            self.website_pub_date = (
                self.website_pub_date
                or (article_publication_date and datetime.strptime(article_publication_date, "%Y-%m-%d"))
    ...
    def recover_pid_v2(self):
        try:
            issue_proc = IssueProc.objects.filter(issue=self.issue).first()
            issue_pid = issue_proc.pid
        except AttributeError:
            issn_id = issue_proc.journal_proc.pid
            year = self.issue.publication_year
            issue_order = str(self.issue.order).zfill(4)
            number = str(self.order or randint(0, 100000)).zfill(5)
            issue_pid = f"{issn_id}{year}{issue_order}"
    ...
    def prepare_sps_package(self, user):
        ...
        for xml_with_pre in XMLWithPre.create(path=self.file.path):
            if self.prepare_xml_file(xml_with_pre) or not self.sps_pkg or not self.sps_pkg.valid_components:
                texts = {
