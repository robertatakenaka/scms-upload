...
class PublicationPreparationException(Exception):
    pass
...
class Package(CommonControlField, ClusterableModel):
    ...
    def xml(self):
        try:
            self.xml_with_pre.tostring(pretty_print=True)
        except Exception as e:
    ...
    def analyze_sps_package(self):
        ...
        if not self.sps_pkg.valid_components:
            for component in self.sps_pkg.components.filter(uri=None):
                errors.append(_("{} is not published on MinIO").format(component.basename))
    ...
    def analyze_result(self, result, rule):
        if rule == choices.STRICT_AUTO_PUBLICATION:
            if critical_errors or errors or self.has_errors:
                result["request_correction"] = True
                # Há erros e é modo rígido, somente publica em QA
                raise PublicationPreparationException(
                    _(
    ...
    def prepare_sps_package(self, user):
        ...
        if not self.sps_pkg:
            raise PublicationPreparationException(_("Unable to prepare the package to publish"))
    ...
    def prepare_to_publish(self, user, operation):
        ...
        try:
            ...
            result = self.analyze_sps_package(user)
        ...
        except PublicationPreparationException as exc:
            logging.exception(exc)
