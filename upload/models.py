    ...
    def split(self, user):
        ...
        with ZipFile(self.file.path) as zf:
            ...
            for key in xmls.keys():
                ...
                for k in files_copy.keys():
                    if k == key:
                        xmls[key].extend(files.pop(k))
...
class Package(CommonControlField, ClusterableModel):
    ...
    xml_errors_percentage = models.DecimalField(
        verbose_name=_("Error percentual"), max_digits=5, decimal_places=2, default=0.00
    )
    ...
    linked = models.ManyToManyField(
        ...
        help_text=_(
            "Related packages which will be published only if all are approved"
        ),
        null=True,
        blank=True,
    )
    ...
    def process_qa_decision(self, user):
        ...
        expected_qa_decision = self.qa_decision
        logging.info(f"prepare_publication on {website_kind} - status: {self.status}")
        self.status = self.qa_decision
        self.save()
        self.prepare_publication(user)
        return {"qa_decision": expected_qa_decision, "website_kind": website_kind}
...
class WorkInProgressPackage(Package):
    class Meta:
        proxy = True
...
class ReadyToPreviewPackage(Package):
    ...
    base_form_class = ReadyToPreviewPackageForm
...
class UploadValidator(CommonControlField):
    ...
    decision_for_critical_errors = models.CharField(
        ...
        default=choices.PS_PENDING_CORRECTION,
    )
