...
from xmlsps import xml_sps_adapter
from xmlsps import v3_gen
...
class XMLVersion(MinioFile):
    ...
    @classmethod
    def create(cls, uri, creator, basename, finger_print):
        obj = cls()
...
class XMLDocPid(CommonControlField):
    ...
    aop_xml_versions = models.ManyToManyField(XMLVersion, related_name='aop_xml_versions')
    xml_versions = models.ManyToManyField(XMLVersion, related_name='xml_versions')
    ...
    def latest_version(self):
        if self.xml_versions and self.xml_versions.count() > 0:
            return self.xml_versions.latest('created')
        if self.aop_xml_versions and self.aop_xml_versions.count() > 0:
            return self.aop_xml_versions.latest('created')
        return None

    def add_version(self, uri, creator, basename, finger_print):
        if not self.latest_version or self.latest_version.finger_print != finger_print:
            version = XMLVersion.create(
                uri, creator, basename, finger_print
            )
            if self.is_aop:
